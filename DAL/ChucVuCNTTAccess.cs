...
namespace DAL
{
    public class ChucVuCNTTAccess : DatabaseAccess
    {
        public List<NhanLucCNTT> HienThiNhanLucCNTT(string nam, string mabm)
        {
            List<NhanLucCNTT> NhanLucCNTT = new List<NhanLucCNTT>();
            OpenConnection();
            ...
            while (reader.Read())
            {
                ...
                NhanLucCNTT CNTT = new NhanLucCNTT();
                CNTT.TenGV = tengv;
                CNTT.TenChucVu = tencv;
                CNTT.TenHocHam = tenhh;
                CNTT.TenHocVi = tenhv;
                CNTT.MaGV = magv;

                NhanLucCNTT.Add(CNTT);
            }
            reader.Close();
